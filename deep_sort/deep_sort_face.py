...
class DeepSortFace(object):
    def __init__(self, model_path, max_dist=0.5, min_confidence=0.3, nms_max_overlap=1.0, max_iou_distance=0.7,
                 max_age=50, n_init=1, nn_budget=100, use_cuda=False):
    ...
    def _get_features_f(self, bbox_xywh, ori_img):
        """
        :param bbox_xywh:
        :param ori_img: cv2 array (h,w,3)
        :return:
        ...
        for box in bbox_xywh:
            x1, y1, x2, y2 = self._xywh_to_xyxy(box)
            im_crops.append((y1,x2,y2,x1))
        if im_crops:
            features = face_recognition.face_encodings(ori_img, known_face_locations=im_crops)
        else:
            features = np.array([])
        return features
